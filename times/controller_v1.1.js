/*
 * $Id: controller_v1.1.js 88168 2012-02-02 21:35:10Z cookn $
...
//set flags for registration tagging
...
if ((/[0-9][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9]\/sunday-review\//.test(nyt_path)) || 
     (/[0-9][0-9][0-9][0-9]\/[0-1][0-9]\/[0-3][0-9]\/opinion\/sunday\//.test(nyt_path))) {
  setSundayReviewInfo();
}
...
// FUNCTIONS
function setRegistrationSection(){
    var section = getMetaTag("WT.cg_n");
    switch(section) {
        case "Member Center": break; //do nothing
        case "Digital Subscription": break; //do nothing
        default:
            addMetaTag("WT.z_rcgn", section);
            addMetaTag("WT.z_rcgs", getMetaTag("WT.cg_s"));
    } 
}

  /**
   *  When OC is present, default SCG to NYT, 
    *   set to IHT for specific OCs 
   */
function setSubcontentGroupByOC() {
    
    if (getQueryParameter("OC") == '') {
        return false;    
    }
    
        // hardcoded as per bugzilla-58282
    var globalOCs = ['4285135516', '1803925390', '1873190447'];
    
    for (i in globalOCs) {
        if (globalOCs[i] == getQueryParameter("OC")) {
            overwriteMetaTag("SCG", "IHT");
        }
    }
    
    if (getMetaTag("SCG") == '') {
        overwriteMetaTag("SCG", "NYT");
    }
...
function getContentGroup(wt_url, wt_domain, wt_path) {
  ...
  if (contentGroup == "") {
   ...
   // crosswordsgames
    ...
    // mostpopular 
	} else if (/^\/most-(popular|popular-searched|popular-movies|popular-emailed|popular-viewed|popular-blogged)\//.test(wt_path)) {
...
function setSundayReviewInfo() {

  overwriteMetaTag("WT.cg_n", "Opinion");
  overwriteMetaTag("WT.cg_s", "Sunday Review");
